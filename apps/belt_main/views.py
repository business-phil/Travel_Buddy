...
from django.db.models import Q  # Import Q to run "OR" query in index
from .models import User, Trip, Guest

def index(request):  # Main page. Redirect to login/reg without session id
...
def create(request):  # Process Add Trip request using TripManager
    current_user = User.objects.get(id=request.session['user_id'])
...
def view(request, trip_id):  # Render View Trip page for individual trip
    ...
    # FUTURE FEATURE: Create boolean to make "Join" link appear if not already the planner or a guest
    context = {
        'trip': trip_query,
        'guests': guest_query
        }
...
def join(request, trip_id):  # Processes Join request using TripManager
